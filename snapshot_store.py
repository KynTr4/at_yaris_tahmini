...
def horse_entity(entry: dict) -> str:
    ...
    if tjk_id not in (None, "", 0, "0"):
        return f"tjk:{tjk_id}"
    digest = hashlib.sha256(normalize_name(entry.get("HORSE_NAME")).encode()).hexdigest()[:20]
...
def parse_race_start(program_date: str, tab: dict) -> str | None:
    ...
    local = base.replace(hour=int(match.group(1)), minute=int(match.group(2)), tzinfo=ISTANBUL)
    return local.astimezone(UTC).isoformat()


def parse_race_no(tab: dict) -> int | None:
    match = re.search(r"(\d+)\s*[.]?\s*Ko", str(tab.get("RACE_NO") or tab.get("RACE_TAB_NAME") or ""), re.I)
...
def parse_agf(value: object) -> tuple[float | None, int | None]:
    ...
    return as_float(percent.group(1)) if percent else None, int(rank.group(1)) if rank else None
                ...
                for entry in tab.get("PROGRAM_LIST", []) or []:
                    ...
                    cursor = connection.execute(
                        """INSERT OR IGNORE INTO program_snapshots(
                           ...
                           VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                        (race_id, horse_id, race_start_at, race_no, captured_at,
                         source_endpoint, source_request_id, as_float(entry.get("START")),
                         as_float(entry.get("WEIGHT")), entry.get("JOCKEY"), entry.get("COACH"),
                         as_float(entry.get("HANDICAP")), race_class, track, surface,
                         distance, horse_name),
                    )
    ...
    try:
        ...
        cursor = connection.execute(
            """INSERT OR IGNORE INTO race_results(
               ...
               VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)""",
            (program["race_id"], horse_id, program["race_start_at"], program["race_no"],
             captured_at, source_endpoint, source_request_id, as_float(result["finish"]),
             result["race_time"], as_float(result["prize"]), None,
             as_float(result["odds"]), status),
        )
        connection.commit()
