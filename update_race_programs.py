...
from discover_horses import i, f, text, record, upsert
...
async def main():
    ...
    with connect(db_path) as db:
        done = db.execute("SELECT status FROM progress WHERE work_type='public_discovery' AND entity_key=? AND endpoint_key='race_program'", (marker,)).fetchone()
    ...
    async with c.open():
        ...
        if payload is None:
            ...
            await c.checkpoint("public_discovery", marker, "race_program", "failed", message="Empty payload")
        ...
        if not cities:
            logger.info(f"No race program found for {today_str}; marking as no_race_today.")
            await c.checkpoint("public_discovery", marker, "race_program", "not_found", message="no_race_today")
        ...
        if not source_request_id:
            raise RuntimeError("Race program response has no immutable source request id")
        with connect(db_path) as db:
        ...
        with connect(db_path) as db:
            db.execute("DELETE FROM race_program_entries WHERE program_date=?", (today_str,))
            for city in cities:
                ...
                for tab in city.get("SUB_TAB", []) or []:
                    ...
                    for entry in tab.get("PROGRAM_LIST", []) or []:
                        ...
                        x = {**hi, **{k: v for k, v in entry.items() if k != "HORSE_INFO"}}
