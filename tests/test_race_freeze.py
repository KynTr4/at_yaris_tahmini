...
class RaceFreezeTests(unittest.TestCase):
    ...
    def test_window_classification(self):
        ...
        self.assertEqual(classify_race(datetime(2030,1,1,13,59,59,tzinfo=timezone.utc),start,False,False,True),"WAITING")
        self.assertEqual(classify_race(datetime(2030,1,1,14,14,59,tzinfo=timezone.utc),start,False,False,True),"PRE_WINDOW")
        self.assertEqual(classify_race(datetime(2030,1,1,14,15,tzinfo=timezone.utc),start,False,False,True),"CAPTURING")
        self.assertEqual(classify_race(datetime(2030,1,1,14,20,tzinfo=timezone.utc),start,False,False,True),"CAPTURING")
        self.assertEqual(classify_race(datetime(2030,1,1,14,29,tzinfo=timezone.utc),start,False,False,True),"FINAL_CAPTURING")
        self.assertEqual(classify_race(datetime(2030,1,1,14,31,tzinfo=timezone.utc),start,False,False,True),"POST_START_RETRY")
        self.assertEqual(classify_race(datetime(2030,1,1,14,32,1,tzinfo=timezone.utc),start,False,False,True),"FAILED")
        self.assertEqual(classify_race(datetime(2030,1,1,14,20,tzinfo=timezone.utc),start,False,False,False),"SOURCE_UNSUPPORTED")

    def test_agf_failure_still_attempts_shadow_mode(self):
        now=datetime(2030,1,1,14,20,tzinfo=timezone.utc);calls=[]
        def fake(script,args,timeout):
            calls.append(script)
            exit_code = 1 if script == "run_agf_update.py" else 0
        ...
        self.assertEqual(calls,["run_agf_update.py","shadow_mode.py"])
    ...
    def test_final_prediction_is_created_once_and_frozen(self):
        ...
        self.assertIn("race-1430",first["capture_races"]);self.assertEqual(calls.count("shadow_mode.py"),1)
        calls.clear();second=process("2030-01-01",datetime(2030,1,1,14,21,tzinfo=timezone.utc),self.db,fake)
        self.assertEqual(second["capture_races"],[]);self.assertEqual(calls,[])
        with sqlite3.connect(self.db) as c:
    ...
    def test_missed_window_never_calls_prediction(self):
        calls=[]
        result=process("2030-01-01",datetime(2030,1,1,14,32,1,tzinfo=timezone.utc),self.db,
                       lambda *args:(calls.append(args) or {"exit_code":0}))
        self.assertEqual(calls,[]);self.assertEqual(result["capture_races"],[])
        with sqlite3.connect(self.db) as c:
