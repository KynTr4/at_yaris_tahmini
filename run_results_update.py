...
REQUIRED_COLUMNS = {
    "race_results": {"race_id", "horse_id", "race_start_at", "captured_at", "finish_position", "result_status"},
    "prediction_results": {"prediction_id", "matched_at"},
    "shadow_monitoring_runs": {"run_id", "run_at", "snapshot_coverage_pass", "pipeline_status", "details_json"},
}
...
def emit(event: str, **fields: Any) -> None:
    print(json.dumps({"event": event, "at": utc_now(), **fields}, ensure_ascii=False), flush=True)
...
def db_state() -> dict[str, Any]:
    ...
    try:
        ...
        for table in ("race_results", "prediction_results"):
            integrity.extend(connection.execute(f"PRAGMA foreign_key_check({table})").fetchall())
        if integrity:
            raise RuntimeError(f"Foreign-key integrity failed: {len(integrity)} violation(s)")
        ...
        return {
            "result_rows": connection.execute("SELECT COUNT(*) FROM race_results").fetchone()[0],
            "prediction_result_rows": connection.execute("SELECT COUNT(*) FROM prediction_results").fetchone()[0],
...
def monitoring_warnings(monitor: dict[str, Any] | None) -> list[str]:
    ...
    try:
        ...
        details = {}; warnings.append("shadow_monitor details_json is invalid")
    ...
    for field in ("prediction_drift_status", "calibration_status", "feature_drift_status"):
...
def base_payload() -> dict[str, Any]:
    return {
        "runner": "results_update", "status": "FAILED", "started_at": utc_now(),
        "ended_at": None, "duration_seconds": 0,
        "update_results_exit_code": 0, "shadow_monitor_exit_code": 0,
        "inserted_results_count": 0, "distinct_result_races_today": 0,
        "matched_predictions_count": 0, "warnings": [], "errors": [],
    }


def default_options() -> argparse.Namespace:
    return argparse.Namespace(date=None, today_tracks=False, country="ALL", track=[],
                              skip_monitor=False, live_status=False)
...
def main(options: argparse.Namespace | None = None) -> int:
    ...
    payload = base_payload(); started = time.monotonic(); service_exit_code = 1
    emit("results_update_started", started_at=payload["started_at"])
    try:
        if getattr(options, "live_status", False):
            live_date = target_date or datetime.now(ZoneInfo("Europe/Istanbul")).date().isoformat()
            previous = read_status_file(LOG_DIR / "live_results_status.json")
            write_live_status(DB_PATH, LOG_DIR / "live_results_status.json", live_date, country, {
                "status": "RUNNING", "started_at": payload["started_at"],
                "last_run_at": previous.get("last_run_at") or previous.get("ended_at"),
                "warnings": [], "errors": [],
            })
        with runner_lock("results_update", skip_if_active=True) as lock:
            if not lock.acquired:
                ...
                service_exit_code = 0
                emit("results_update_skipped", reason="already_running", owner=lock.metadata)
            else:
                ...
                emit("step_completed", script="update_results.py", exit_code=update["exit_code"],
                     started_at=update["started_at"], ended_at=update["ended_at"],
                     duration_seconds=update["duration_seconds"])
                if int(update["exit_code"]) != 0:
                    payload["errors"].append(f"update_results.py exit_code={update['exit_code']}")
                    ...
                    if getattr(options, "skip_monitor", False):
                        final = after_update
                        payload["distinct_result_races_today"] = int(final["distinct_result_races_today"])
                        payload["warnings"] = list(dict.fromkeys(payload["warnings"]))
                        payload["status"] = "WARNING" if payload["warnings"] else "SUCCESS"
                        service_exit_code = 0
                    else:
                        monitor_before_id = (after_update.get("monitor") or {}).get("run_id")
                        monitor_result = run_step("shadow_monitor.py", [], 1800)
                        payload["shadow_monitor_exit_code"] = int(monitor_result["exit_code"])
                        forward_step_output(monitor_result)
                        emit(
                            "step_completed", script="shadow_monitor.py", exit_code=monitor_result["exit_code"],
                            started_at=monitor_result["started_at"], ended_at=monitor_result["ended_at"],
                        ...
                        payload["matched_predictions_count"] = max(
                            0, int(final["prediction_result_rows"]) - int(after_update["prediction_result_rows"])
                        )
                        latest_monitor = final.get("monitor")
                        monitor_completed = bool(
                            latest_monitor and latest_monitor.get("run_id") != monitor_before_id
                        )
                        monitor_exit = int(monitor_result["exit_code"])
                        has_traceback = "Traceback" in str(monitor_result.get("stderr") or "")
                        monitoring_only_failure = monitor_exit == 1 and monitor_completed and not has_traceback
                        if monitor_exit != 0 and not monitoring_only_failure:
                            ...
                            payload["warnings"].extend(monitoring_warnings(latest_monitor))
                            if monitor_exit != 0:
        ...
        if getattr(options, "live_status", False):
            ...
            try:
                write_live_status(DB_PATH, LOG_DIR / "live_results_status.json", live_date, country, {
                    ...
                    "ended_at": payload["ended_at"], "duration_seconds": payload["duration_seconds"],
                    "warnings": payload["warnings"], "errors": payload["errors"],
                })
            except Exception as exc:
                traceback.print_exc(file=sys.stderr)
                payload["status"] = "FAILED"; payload["errors"].append(f"live status write failed: {exc}")
                service_exit_code = 1
        try:
            ...
            service_exit_code = 1
        emit(
            "results_update_finished", status=payload["status"], ended_at=payload["ended_at"],
            duration_seconds=payload["duration_seconds"], exit_code=service_exit_code,
