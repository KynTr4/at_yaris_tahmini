...
def filter_program_horses(horses, country="ALL", tracks=None, completed_tracks=None):
    ...
    return [horse for horse in horses if (
        track_in_country(clean_track(horse.get("city_name")), country)
        and (not selected_tracks or clean_track(horse.get("city_name")) in selected_tracks)
...
async def fetch_endpoint(c, key, path, entity, params, force=False):
    ...
    if PUBLIC_ONLY_MODE and restricted:
        return None
    try:
        return await c.request(key, path, params=params, entity_key=entity, store_raw=True)
...
def publish_coverage(db_path, target_date: date):
    ...
    logger.info("RESULTS_COVERAGE %s", json.dumps(coverage["tracks"], ensure_ascii=False))
...
async def update_date(target_date: date, country: str = "ALL", tracks: list[str] | None = None,
    ...
    if not horses:
        logger.info("No horses found in %s race program. Results cannot be updated.", today_str)
    ...
    if skip_completed_tracks:
        ...
        completed_tracks = {
            clean_track(row["track"]) for row in coverage["tracks"]
    ...
    for horse in scoped_horses:
        ...
        if policy == "unsupported":
            logger.warning(
                ...
                clean_track(horse.get("city_name")), horse.get("horse_name"), horse.get("tjk_id"),
            )
            continue
        supported_horses.append(horse)
    horses = supported_horses
        
    logger.info(f"Found {len(horses)} horses in today's race program. Checking if results are already in DB...")
    ...
    async with c.open():
        for entity, tjk_id, horse_id, name, track in missing_results:
            ...
            response = await fetch_endpoint(c, "GET:Tjk/Get", "Tjk/Get", entity, {"p_iTjkId": tjk_id})
            if response is None:
                continue
            ...
            try:
                ...
                if not source_request_id:
                    raise RuntimeError("Result response has no immutable source request id")
                with connect(db_path) as db:
                    capture = db.execute(
                ...
                if inserted:
                    ...
                    logger.warning(
                        "RESULT_NOT_APPENDED track=%s horse=%s entity=%s "
                        "reason=already_inserted_or_ambiguous_mapping",
                        track, name, entity,
                    )
            except Exception as e:
    ...
    logger.info(f"Completed update_results.py. Successfully updated results for {success_count}/{len(missing_results)} horses.")
...
async def main(target_date: date | None = None, lookback_days: int = 1, country: str = "ALL",
               tracks: list[str] | None = None, today_tracks: bool = False):
    ...
    for selected_date in dates:
        coverages.append(await update_date(selected_date, country, tracks, today_tracks))
...
if __name__ == "__main__":
    ...
    if args.today_tracks and selected is None:
        selected = date.today()
    asyncio.run(main(selected, args.lookback_days, args.country, args.track, args.today_tracks))
