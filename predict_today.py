...
def normalize_prediction_frame(df):
    ...
    for prob_col, norm_col in pairs:
        ...
        for rid, group in df.groupby("race_id"):
            ...
            if total > 0:
                df.loc[idx, norm_col] = (
                    pd.to_numeric(group[prob_col], errors="coerce").fillna(0.0) / total
                )
            elif len(group) > 0:
...
def main():
    ...
    if not os.path.exists(final_parquet):
        ...
        log_failure(
            final_parquet, "MissingDataset", "Final dataset Parquet file not found"
        )
        return 1
    ...
    try:
        ...
        local_dates = (
            ...
            .dt.tz_convert(ZoneInfo("Europe/Istanbul"))
            .dt.date.astype(str)
        )
    ...
    if df_today.empty:
        logger.warning(
            f"No races found in final dataset for today ({today_str}). Predictions cannot be generated."
        )
        write_prediction_status(
            "no_race_today", f"No races found in final dataset for {today_str}"
        )
        return 0
    ...
    if "xgb" in models:
        try:
            ...
            logger.info(
                "XGBoost production model predictions generated without fallback."
            )
        except Exception as e:
            logger.error(f"XGBoost production prediction failed: {e}")
            log_failure(
                "models/xgboost_production.pkl", "XGBoostPredictionError", str(e)
            )
        ...
        log_failure(
            ...
            "ModelNotLoaded",
            "XGBoost production model was not loaded",
        )
        write_prediction_status(
            "xgboost_model_not_loaded", "XGBoost production model was not loaded"
        )
        return 1
    ...
    predictions["ensemble_norm_prob"] = (
        predictions["lr_norm_prob"]
        + predictions["xgb_norm_prob"]
        + predictions["cb_norm_prob"]
    ) / 3.0

    # 6. Write predictions directly to SQLite (no CSV/parquet output)
    # predicted_rank hesaplanmali records dongusunden ONCE
    predictions["predicted_rank"] = (
        predictions.groupby("race_id")["ensemble_norm_prob"]
    ...
    for _, row in predictions.iterrows():
        # race_no and track from df_today
        df_row = df_today[df_today["race_id"] == row["race_id"]]
        track = (
            str(df_row["track"].iloc[0])
            if not df_row.empty and "track" in df_row.columns
            else ""
        )
        race_no_val = (
            int(df_row["race_no"].iloc[0])
            if not df_row.empty
            and "race_no" in df_row.columns
            and pd.notna(df_row["race_no"].iloc[0])
            else None
        )
        race_start = (
            str(df_row["race_start_at"].iloc[0])
            if not df_row.empty and "race_start_at" in df_row.columns
            else None
        )
        records.append(
            {
                ...
                "lr_prob": float(row.get("lr_norm_prob", 0) or 0),
                "xgb_prob": float(row.get("xgb_norm_prob", 0) or 0),
                "cb_prob": float(row.get("cb_norm_prob", 0) or 0),
                "ensemble_prob": float(row.get("ensemble_norm_prob", 0) or 0),
                "predicted_rank": int(row.get("predicted_rank", 0) or 0),
            }
        )
    ...
    try:
        ...
        conn.executemany(
            """INSERT OR REPLACE INTO model_prediction_runs
               (prediction_date, race_id, horse_id, horse_name, track, race_no, race_start_at,
                lr_prob, xgb_prob, cb_prob, ensemble_prob, predicted_rank)
               VALUES(:prediction_date,:race_id,:horse_id,:horse_name,:track,:race_no,:race_start_at,
                      :lr_prob,:xgb_prob,:cb_prob,:ensemble_prob,:predicted_rank)""",
            records,
        )
        conn.commit()
        logger.info(
            "Predictions written to SQLite model_prediction_runs: %d rows", len(records)
        )
    except Exception as e:
        logger.error("Failed to write predictions to SQLite: %s", e)
        log_failure(today_str, "SQLiteWriteError", str(e))
        write_prediction_status("sqlite_write_error", str(e))
        return 1
    finally:
        if conn is not None:
            conn.close()
    ...
    write_prediction_status(
        ...
        f"Predictions written to SQLite (model_prediction_runs). {len(records)} rows for {predictions['race_id'].nunique()} races.",
        rows=len(records),
        races=predictions["race_id"].nunique(),
    )
    ...
    for rid, group in predictions.groupby("race_id"):
        ...
        for rank, (idx, r) in enumerate(sorted_grp.head(3).iterrows(), 1):
            print(
                f"  {rank}. {r['horse_name']} (CB: {r['cb_norm_prob']:.2%}, XGB: {r['xgb_norm_prob']:.2%}, LR: {r['lr_norm_prob']:.2%})"
            )
    print("=" * 50 + "\n")
    return 0
